...
(() => {
  ...
  var require_source_map_resolve = __commonJS({
    "node_modules/source-map-resolve/index.js"(exports, module) {
      ...
      function resolveSources(map3, mapUrl, read2, options, callback) {
        ...
        resolveSourcesHelper(map3, mapUrl, options, function(fullUrl, sourceContent, index2) {
          ...
          if (typeof sourceContent === "string") {
            ...
            read2(readUrl, function(error, source) {
              result.sourcesContent[index2] = error ? error : String(source);
              done();
            });
          }
        });
      ...
      function resolveSourcesHelper(map3, mapUrl, options, fn) {
        ...
        var sourceContent;
        var sourceRoot;
        for (var index2 = 0, len = map3.sources.length; index2 < len; index2++) {
          sourceRoot = null;
          ...
          if (sourceRoot === null || sourceRoot === "") {
            fullUrl = resolveUrl(mapUrl, map3.sources[index2]);
          } else {
            fullUrl = resolveUrl(mapUrl, sourceRoot.replace(endingSlash, "/"), map3.sources[index2]);
          }
          sourceContent = (map3.sourcesContent || [])[index2];
  ...
  function parse(query2, options, { auto = true } = {}) {
    const next = (query3) => {
  ...
  function completions(keyseq, map3) {
    return new Map(
      filter(map3.entries(), ([ks, _maptarget]) => prefixes(keyseq, ks))
    );
  }
  ...
  function translateKeysUsingKeyTranslateMap(keyEvents, keytranslatemap) {
    for (let index2 = 0; index2 < keyEvents.length; index2++) {
      if (!keyEvents[index2].translated) {
        keyEvents[index2] = keyEvents[index2].translate(keytranslatemap);
        keyEvents[index2].translated = true;
      }
    }
  }
    ...
    if (obj !== void 0 && obj !== null && target.length) {
      if (obj["\u{1F577}\u{1F577}INHERITS\u{1F577}\u{1F577}"] === void 0) {
        ...
        return getDeepProperty(
          mergeDeepCull(get2(obj["\u{1F577}\u{1F577}INHERITS\u{1F577}\u{1F577}"]), obj)[target[0]],
          target.slice(1)
        );
      }
    } else {
  ...
  async function getAsync(target_typed, ...target) {
    if (INITIALISED) {
      ...
      return new Promise(
        (resolve) => WAITERS.push(() => resolve(get2(target_typed, ...target)))
      );
    }
  }
  async function init() {
  ...
  // src/completions.ts
  var DEFAULT_FAVICON = browser.runtime.getURL(
  ...
  var CompletionSourceFuse = class extends CompletionSource {
    ...
    updateChain(exstr = this.lastExstr, options = this.options) {
      ...
      const [prefix, query2] = this.splitOnPrefix(exstr);
      if (prefix) {
      ...
      if (query2) {
        this.setStateFromScore(this.scoredOptions(query2));
      } else {
    ...
    splitOnPrefix(exstr) {
      for (const prefix of this.prefixes) {
        if (exstr.startsWith(prefix)) {
          const query2 = exstr.replace(prefix, "");
          return [prefix, query2];
        }
      }
      return [void 0, void 0];
    }
    scoredOptions(query2) {
      const searchThis = this.options.map((elem, index2) => ({
        index: index2,
        fuseKeys: elem.fuseKeys
      }));
      this.fuse = new Fuse(searchThis, this.fuseOptions);
      return this.fuse.search(query2).map((result) => {
        const index2 = toNumber(result.item.index);
        return {
          index: index2,
          option: this.options[index2],
          score: result.score
        };
      });
    }
    setStateFromScore(scoredOpts, autoselect = false) {
      ...
      for (const [index2, option] of enumerate(this.options)) {
        if (matches.includes(index2))
          option.state = "normal";
        else {
          option.state = "hidden";
      ...
      if (this.sortScoredOptions) {
        const sorted_options = matches.map((index2) => this.options[index2]);
  ...
  var everything = new ProgramMetadata(/* @__PURE__ */ new Map([
    ["src/excmds.ts", new FileMetadata(/* @__PURE__ */ new Map([]), /* @__PURE__ */ new Map([
      ...
      ["native", new SymbolMetadata("Tells you if the native messenger is installed and its version.\n\nFor snap, flatpak, and other sandboxed installations, additional setup is required \u2013 see https://github.com/tridactyl#extra-features-through-native-messaging.", new FunctionType([], new TypeReferenceType("Promise", [new AnyType(true, false)], false, false), false, false), false)],
      ...
      ["tgroupswitch", new SymbolMetadata("Switch to a different tab group, hiding all other tabs.", new FunctionType([new StringType(false, false)], new TypeReferenceType("Promise", [new StringType(false, false)], false, false), false, false), false)],
      ["tgrouplast", new SymbolMetadata("Switch to the previously active tab group.", new FunctionType([], new TypeReferenceType("Promise", [new AnyType(true, false)], false, false), false, false), false)],
      ...
      ["tgroupclose", new SymbolMetadata("Close the current tab group.\n\nFirst switch to the previously active tab group. Do nothing if there is only\none tab group.", new FunctionType([], new TypeReferenceType("Promise", [new AnyType(true, false)], false, false), false, false), false)],
      ["tgroupmove", new SymbolMetadata("Move the current tab to another tab group.", new FunctionType([new StringType(false, false)], new TypeReferenceType("Promise", [new StringType(false, false)], false, false), false, false), false)],
    ...
    ["src/lib/config.ts", new FileMetadata(/* @__PURE__ */ new Map([["default_config", new ClassMetadata(/* @__PURE__ */ new Map([
      ...
      ["jsurls", new SymbolMetadata('Like [[searchurls]] but must be a Javascript function that takes one argument (a single string with the remainder of the command line including spaces) and maps it to a valid href that will be followed, e.g. `set jsurls.googleloud query => "https://google.com/search?q=" + query.toUpperCase()`\n\nNB: the href must be valid, i.e. it must include the protocol (e.g. "http://") and not just be e.g. "www.".', new ObjectType(/* @__PURE__ */ new Map([]), false, false), false)],
...
statistics, so somewhere around 10k samples.`, new StringType(false, false), false)],
      ...
      ["get", new SymbolMetadata("Get the value of the key target.\n\nIf the user has not specified a key, use the corresponding key from\ndefaults, if one exists, else undefined.", new FunctionType([new UnionType([
        ...
        new TypeReferenceType("... 84 more ...", [], false, false),
...
When adding updaters, don't forget to set("configversion", newversionnumber)!`, new FunctionType([], new TypeReferenceType("Promise", [new BooleanType(false, false)], false, false), false, false), true)],
  ...
  var AproposCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      const flags = {
        "-a": (options, query3) => options.concat(
          Object.keys(exaliases).filter(
            (alias) => (alias + expandExstr(alias) + excmds.getFunction(expandExstr(alias))).toLowerCase().includes(query3)
          ).map((alias) => {
        ...
        "-b": (options, query3) => options.concat(
          Object.keys(bindings).filter(
            (binding) => (binding + bindings[binding]).toLowerCase().includes(query3)
          ).map(
            (binding) => new AproposCompletionOption(
              binding,
        ...
        "-e": (options, query3) => options.concat(
          fns.filter(
            ([name, fn]) => !fn.hidden && (name + fn.doc).toLowerCase().includes(query3)
          ).map(
            ([name, fn]) => new AproposCompletionOption(
              name,
              `Excmd. ${fn.doc}`,
              "-e"
            )
        ...
        "-s": (options, query3) => options.concat(
          Object.keys(settings).filter(
            (x) => (x + default_config2.getMember(x).doc).toLowerCase().includes(query3)
          ).map((setting) => {
      ...
      const args = query2.split(" ");
      let opts = [];
      if (Object.keys(flags).includes(args[0])) {
        opts = flags[args[0]](opts, args.slice(1).join(" "));
      } else {
        opts = Object.keys(flags).reduce(
          (acc, curFlag) => flags[curFlag](acc, query2),
          []
        );
      }
      this.options = opts;
      this.options.sort(
        (compopt1, compopt2) => compopt1.name.localeCompare(compopt2.name)
      );
      return this.updateChain();
    }
    updateChain() {
  ...
  var BindingsCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      this.lastExstr = exstr;
      let options = "";
      let [prefix, query2] = this.splitOnPrefix(exstr);
      const args = query2 ? query2.split(/\s+/) : [];
      ...
      if (prefix.trim().endsWith("url")) {
        ...
        if (args.length === 0) {
          ...
          this.options = Object.keys(patterns).filter((pattern) => pattern.startsWith(urlPattern)).sort().map(
            (pattern) => new BindingsCompletionOption(pattern, {
              name: pattern,
              value: "",
              mode: "URL Pattern"
            })
      ...
      if (args.length === 1 && args[0].startsWith("--m")) {
        ...
        if ("--mode".includes(margs[0])) {
          ...
          this.options = modes.filter((k) => k.startsWith(modeStr)).map(
            (name) => new BindingsCompletionOption(
              options + "--mode=" + name,
              {
                name,
                value: "",
      ...
      query2 = args.join(" ").toLowerCase();
      this.options = Object.keys(bindings).filter((x) => x.toLowerCase().startsWith(query2)).sort().map(
        (keystr) => new BindingsCompletionOption(
          options + keystr,
          {
            name: keystr,
  ...
  async function queryAndURLwrangler(query) {
    ...
    const searchurls = get2("searchurls");
    ...
    const enginename = get2("searchengine");
    if (enginename) {
  ...
  var BmarkCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      if (query2.startsWith("-t ")) {
        option = "-t ";
        query2 = query2.slice(3);
      }
      if (query2.startsWith("-c")) {
        const args = query2.split(" ");
        option += args.slice(0, 2).join(" ");
        option += " ";
        query2 = args.slice(2).join(" ");
      }
      this.completion = void 0;
      this.options = (await getBookmarks(query2)).slice(0, 10).map((page) => new BmarkCompletionOption(option + page.url, page));
      this.lastExstr = prefix + query2;
      return this.updateChain();
    }
    ...
    updateChain() {
      const query2 = this.splitOnPrefix(this.lastExstr)[1];
  ...
  var ExcmdCompletionSource = class extends CompletionSourceFuse {
    ...
    async updateOptions(exstr = "") {
      ...
      this.options = this.scoreOptions(
        fns.filter(([name, fn]) => !fn.hidden && name.startsWith(exstr)).map(([name, fn]) => new ExcmdCompletionOption(name, fn.doc))
      );
      ...
      const partial_options = this.scoreOptions(
        fns.filter(
          ([name, fn]) => !fn.hidden && name.includes(exstr) && !seen.has(name)
  ...
  var CompositeCompletionSource = class extends CompletionSourceFuse {
    ...
    select(option) {
      this.completion = this.lastExstr.replace(
    ...
    async updateOptions(exstr = "") {
      ...
      this.options = this.scoreOptions(
        fns.filter(
          ...
          ([name, fn]) => new ExcmdCompletionOption(
            name,
            fn.doc
          )
        )
      );
      const exaliases = Object.keys(get2("exaliases")).filter(
        (a) => a.startsWith(end_exstr)
      );
  ...
  var ExtensionsCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      this.options = this.scoreOptions(
        extensions.filter((extension) => extension.name.startsWith(query2)).map(
          (extension) => new ExtensionsCompletionOption(
            extension.name,
    ...
    try {
      ...
      if (actualVersion !== void 0) {
        if ((0, import_semver_compare.default)(version, actualVersion) > 0) {
          if (interactive)
            logger2.error(
              "# Please update to native messenger " + version + ", for example by running `:updatenative`."
            );
          return false;
        }
        return true;
  ...
  var GotoCompletionSource = class extends CompletionSourceFuse {
    ...
    async updateOptions(exstr = "") {
      ...
      if (this.options.length < 1) {
        this.options = (await messageOwnTab(
          "excmd_content",
          "getGotoSelectors",
          []
        )).sort((a, b) => a.y - b.y).map((heading) => {
    ...
    async filter(exstr) {
      ...
      if (metaRules[ruleName]) {
        this.options = this.options.concat(
          Object.keys(metaRules[ruleName]).filter((s) => s.startsWith(subRule)).map(
      ...
      if (potentialRules[ruleName]) {
        this.options = this.options.concat(
          Object.keys(potentialRules[ruleName].options).filter((s) => s.startsWith(subRule)).map(
            (s) => new GuisetCompletionOption(`${ruleName} ${s}`, s)
          )
        );
      }
  ...
  var HelpCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      const flags = {
        "-a": (options, query3) => options.concat(
          Object.keys(exaliases).filter((alias) => alias.startsWith(query3)).map((alias) => {
            const cmd = expandExstr(alias);
            const doc = (excmds.getFunction(cmd) || {}).doc || "";
        ...
        "-b": (options, query3) => options.concat(
          Object.keys(bindings).filter((binding) => binding.startsWith(query3)).map(
            (binding) => new HelpCompletionOption(
              binding,
              `Normal mode binding for \`${bindings[binding]}\``,
              "-b"
            )
        ...
        "-e": (options, query3) => options.concat(
          fns.filter(
            ([name, fn]) => !fn.hidden && name.startsWith(query3)
          ).map(
            ([name, fn]) => new HelpCompletionOption(
              name,
              `Excmd. ${fn.doc}`,
              "-e"
            )
        ...
        "-s": (options, query3) => options.concat(
          Object.keys(settings).filter((x) => x.startsWith(query3)).map((setting) => {
            const member = default_config2.getMember(setting);
            let doc = "";
            if (member !== void 0) {
              doc = member.doc;
  ...
  var HistoryCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      if (prefix === "tabopen ") {
        ...
        if (query2.startsWith("-b")) {
          const args = query2.split(" ");
          options = args.slice(0, 1).join(" ");
          query2 = args.slice(1).join(" ");
        }
      } else if (prefix === "winopen " && query2.startsWith("-private")) {
        options = "-private";
        query2 = query2.substring(options.length);
      }
      options += options ? " " : "";
    ...
    async scoreOptions(query2, n) {
      if (!query2 || get2("historyresults") === 0) {
        return (await getTopSites()).slice(0, n);
      } else {
        return (await getCombinedHistoryBmarks(query2)).slice(0, n);
      }
    }
  };
  ...
  function computeDate(session) {
    let howLong = Math.round(
      (new Date() - session.lastModified) / 1e3
    );
    let qualifier = "s";
  ...
  var SettingsCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      if (prefix === "seturl " || prefix === "unseturl " || prefix === "viewconfig " && (query2.startsWith("--user") || query2.startsWith("--default"))) {
        const args = query2.split(" ");
        options = args.slice(0, 1).join(" ");
        query2 = args.slice(1).join(" ");
      }
      ...
      this.options = Object.keys(settings).filter((x) => x.startsWith(query2)).sort().map((setting) => {
  ...
  var Marker = class {
    constructor(ownerName, functionName, active = performanceApiAvailable() && get2("perfcounters") === "true", metricName = new MetricName(
      ownerName,
      functionName
    )) {
      this.active = active;
  ...
  var BufferCompletionOption = class extends CompletionOptionHTML {
    constructor(value, tab, isAlternative = false, container) {
      ...
      if (tab.discarded)
        preplain += "D";
      if (get2("completions", "Tab", "statusstylepretty") === "true") {
        if (tab.pinned)
          pre += "\u{1F4CC}";
        if (tab.audible)
          pre += "\u{1F50A}";
  ...
  var BufferCompletionSource = class extends CompletionSourceFuse {
    ...
    scoredOptions(query2, options = this.options) {
      const args = query2.trim().split(/\s+/gu);
      if (args.length === 1) {
        if (Number.isInteger(Number(args[0]))) {
          let index2 = Number(args[0]) - 1;
          if (Math.abs(index2) < options.length) {
            index2 = index2.mod(options.length);
            return this.TabscoredOptionsStartsWithN(index2, options);
          }
        } else if (args[0] === "#") {
          for (const [index2, option] of enumerate(options)) {
      ...
      return super.scoredOptions(query2);
    }
    ...
    async fillOptions() {
      ...
      container_all.forEach(
        (elem) => container_map.set(elem.cookieStoreId, elem)
      );
    ...
    async updateOptions(exstr = "") {
      ...
      if (prefix === "tabmove")
        this.shouldSetStateFromScore = !/^[+-][0-9]+$/.exec(query2);
  ...
  var TabAllCompletionSource = class extends CompletionSourceFuse {
    ...
    async getWindows() {
      const windows2 = await browserBg.windows.getAll();
      const response = {};
      windows2.forEach((win) => response[win.id] = win);
      return response;
    }
    async reactToTabChanges() {
      await this.updateOptions(this.lastExstr);
    ...
    async updateOptions(exstr = "") {
      ...
      const [tabs, windows2] = await Promise.all([tabsPromise, windowsPromise]);
      const options = [];
      tabs.sort((a, b) => {
      ...
      for (const tab of tabs) {
        ...
        options.push(
          new TabAllCompletionOption(
            ...
            tab,
            winindex,
            await getFromId(tab.cookieStoreId),
            windows2[tab.windowId].incognito
          )
        );
      }
      this.completion = void 0;
  ...
  var ThemeCompletionSource = class extends CompletionSourceFuse {
    ...
    async updateOptions(exstr = "") {
      ...
      const themes = staticThemes.concat(
        Object.keys(await get2("customthemes"))
      );
      const [prefix, query2] = this.splitOnPrefix(exstr);
      if (prefix) {
        if (this.state === "hidden") {
      ...
      this.options = this.scoreOptions(
        themes.filter((name) => name.startsWith(query2)).map((name) => new ThemeCompletionOption(name))
      );
      this.options.forEach((o2) => o2.state = "normal");
      return this.updateChain();
    }
  ...
  // src/completions/TabHistory.ts
  var TabHistoryCompletionOption = class extends CompletionOptionHTML {
    constructor(value, tab) {
      super();
      this.value = value;
      this.fuseKeys = [];
      this.fuseKeys.push(this.value, tab.title);
      const index2 = tab.id ? tab.id : "";
      const timeSpan = tab.formatTimeSpan;
      this.fuseKeys.push(String(index2));
      this.html = html`<tr class="TabHistoryCompletionOption option">
            <td class="prefix">${index2}</td>
            <td class="container"></td>
  ...
  var TabHistoryCompletionSource = class extends CompletionSourceFuse {
    constructor(_parent) {
    ...
    makeTree(nodes, parentId = null, level = 0) {
      return nodes.filter((node) => node["parent"] === parentId).reduce(
        (tree, node) => [
          ...tree,
          {
            ...node,
    ...
    async updateOptions(exstr = "") {
      ...
      let history2 = await browserBg.sessions.getTabValue(tab[0].id, "history");
      if (!history2)
        history2 = { list: [] };
      const tree = this.makeTree(history2["list"]);
      history2["list"] = this.flattenTree(tree[0]).reverse();
      this.addIndicies(history2["list"]);
      this.addFormatTimeSpan(history2["list"]);
      this.options = this.scoreOptions(
        history2["list"].map(
          (item) => new TabHistoryCompletionOption(item.href, {
            href: item.href,
            id: item.index,
            title: item.title,
            prefix: item.prefix,
  ...
  var WindowCompletionSource = class extends CompletionSourceFuse {
    ...
    async updateOptions(exstr = "") {
      ...
      this.options = (await browserBg.windows.getAll({ populate: true })).filter((win) => !(excludeCurrentWindow && win.focused)).map(
        (win) => {
  ...
  var ProxyCompletionOption = class extends CompletionOptionHTML {
    constructor(value, proxy) {
      super();
      this.value = value;
      this.fuseKeys = [];
  ...
  var ProxyCompletionSource = class extends CompletionSourceFuse {
    constructor(_parent) {
      super(
    ...
    async filter(exstr) {
      ...
      const proxies = get2("proxies");
      if (!proxies) {
        this.options = [];
        return this.updateChain();
      }
      query2 = args.join(" ").toLowerCase();
      this.options = Object.keys(proxies).filter((x) => x.toLowerCase().startsWith(query2)).sort().map(
        (key) => new ProxyCompletionOption(
          key,
          {
            name: key,
  ...
  (function(window2) {
    ...
    if (typeof window2.html === "undefined") {
      ...
      window2.html = function(strings, ...values) {
        ...
        function replaceSubstitution(match, index2) {
          return values[parseInt(index2, 10)];
        }
        let str = strings[0];
          ...
          if (node.attributes) {
            ...
            for (const attribute of attributes) {
              ...
              if (name && value.indexOf(SUBSTITUTION_INDEX) !== -1) {
                ...
                value = value.replace(SUBSTITUTION_REGEX, function(match, index2, offset) {
                  if (isRejected) {
                    return "";
                  }
                  let substitutionValue = values[parseInt(index2, 10)];
  ...
  var TabGroupCompletionOption = class extends CompletionOptionHTML {
    constructor(group, tabCount) {
      ...
      this.html = html`<tr class="TabGroupCompletionOption option">
            ...
            <td class="tabcount">${tabCount} tab${tabCount !== 1 ? "s" : ""}</td>
        </tr>`;
    }
  };
  var TabGroupCompletionSource = class extends CompletionSourceFuse {
    constructor(_parent) {
      super(
    ...
    async updateOptions() {
      ...
      this.options = await Promise.all(
        otherGroups.map(async (group) => {
          ...
          const o2 = new TabGroupCompletionOption(group, tabCount);
          o2.state = "normal";
          return o2;
        })
      );
  ...
  function enableCompletions() {
    if (!commandline_state.activeCompletions) {
      ...
      commandline_state.activeCompletions.forEach(
        (comp) => fragment.appendChild(comp.node)
      );
      ...
      if (response.value) {
        ...
        if (response.value.startsWith("ex.")) {
          ...
          QUEUE[QUEUE.length - 1].then(() => {
            QUEUE.push(
              (async () => commandline_state.fns[funcname](
                args.length === 0 ? void 0 : args.join(" ")
              ))()
            );
  ...
  function refresh_completions(exstr) {
    if (!commandline_state.activeCompletions)
      enableCompletions();
    return Promise.all(
      commandline_state.activeCompletions.map(
        (comp) => comp.filter(exstr).then(() => {
  ...
  async function history(n) {
    ...
    const matches = reverse_default(
      uniq_default(reverse_default(await getAsync2("cmdHistory")))
  ...
  addListener(
    "commandline_cmd",
    attributeCaller(commandline_state.fns)
  );
