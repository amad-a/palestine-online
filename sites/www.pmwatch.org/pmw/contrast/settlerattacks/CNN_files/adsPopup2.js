var adsPF,adsC,adsD1=adsD.getTime(),adsCS=1,adsPm=new Array(),adsN6=adsUA.indexOf("Netscape6")!=-1,adsN6O=0,adsP1=new Array(),adsAO=1
var adsW,adsH,adsWin=new Array(),adsA7=adsUA.indexOf("AOL 7")!=-1,adsC2=adsUA.indexOf("CS 2000")!=-1||adsUA.indexOf("CS2000")!=-1,adsInt,adsDm=""
function adSetPopDm(v){adsDm=escape(v)} 
function adSetPopN6(v){if (adsN6){adsN6O=escape(v)}}
...
if (!adsOld&&!adsOp&&!(adsUA.indexOf('NT')!=-1&&!adsA7&&adsUA.indexOf('Gecko')==-1&&(adsC2||adsAOL))&&(c!=-1||(!adsAOL&&!adsC2&&!(adsN6&&adsN6O==0)))&&!(adsAO==1&&adsAOL)){adsPF=f
adsW=w
adsH=h
...
document.write(adsSL+adsHt1+"/html/"+m+"/"+adsScr+"/"+adsExt+"?"+adsNMSG+"&height="+h+"&width="+w+"&target=_blank&CT=J"+adsTz+"&htmlpre=adsObj"+z+"%3d%27&xlnl=%5cn&xltick=%5c%27&ctype=application/x-javascript&htmlsuf=%27%3badsPopup%28"+z+"%29"+adsScS+adsSr+adsOt+"'>")
...
document.cookie="adsPopup"+x+"="+escape(adsD1)+"; path=/; domain="+dm+exp
...
if (b){adsWin[b].focus()}
else{if (adsN6){setTimeout('adsN6F()',1000)}else
if (!adsN6){adsWin[b].blur()
...
if (adsA7){if (adsWin[1]){clearInterval(adsInt);d=1}}else{d=1}
if (d){x=parseInt(x)
...
if (adsN6){setTimeout('adsN6F()',1000)}else
if (!adsN6){adsWin[x].blur()}
w.adsObj0=''}}}}}
